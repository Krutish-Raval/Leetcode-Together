...
import { fetchAllFriends } from "../services/api_user.js";
    ...
    //   console.log(friendsData)
      ...
      if (friendsList.length > 0) {
        const standingsData = await Promise.allSettled(
          friendsList.map((friend) => fetchFriendsPerformance(contestName, friend.leetcodeId))
        );
        ...
        const mergedStandings = validStandings.map((stand) => {
            ...
            return { ...stand, friendName: friend ? friend.friendName : "Unknown" };
        });
            ...
            <tbody>
              {processedStandings.map((friendData, idx) => (
                <tr key={idx} className="border-b border-gray-700 text-center hover:bg-[#252525]">
                  ...
                  <td className="p-3">{friendData.old_rating ? Math.round(friendData.old_rating) : "N/A"}</td>
                  <td className="p-3">{friendData.new_rating ? Math.round(friendData.new_rating) : "N/A"}</td>
